...
for attack in human_attacks:
    attack_data = human_part.filter(lambda x: x["attack"] == attack).shuffle(seed=42)
...
print("Общий размер датасета test:", len(test)) # Общий размер датасета test: 7956
print("Общий размер датасета: train", len(train)) # Общий размер датасета: train 31968
